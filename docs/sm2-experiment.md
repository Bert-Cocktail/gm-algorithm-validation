...
当前已完成向量签名验证、PEM 密钥生成、文件签名和验签、公钥加密、私钥解密及三种密文格式转换，并已接入统一 `runner.py`。尚未实现本地 ACVP 风格 SM2 请求。
...
下一阶段可研究 SM2 本地 ACVP 风格接口、更多与现行 `sm2p256v1` 精确匹配的公开向量，以及 SM2 密钥交换实验。
